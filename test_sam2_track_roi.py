...
VIDEO_PATH       = 'datasets/videos/swim_70.mp4'
...
MASK_DET_IOU_THRESHOLD = 0.1       # SAM2 MASK BBOX / YOLO DET 결과 비교 [추적 객체의 사람 판단 / 신규 객체 후보 등록 시 기존 객체와의 매칭 판단에 사용]
...
def is_bbox_in_roi(bbox, polygon):
    """bbox 중심점이 polygon 내부에 있는지 검사. polygon이 None이면 항상 True (ROI 미적용)."""
...
def select_roi(video_path):
    """첫 프레임에서 마우스 클릭으로 ROI 폴리곤 선택. 'q'로 종료. 점 3개 미만이면 None 반환."""
    temp_cap = cv2.VideoCapture(video_path)
    ret, init_frame = temp_cap.read()
    temp_cap.release()
    if not ret:
        print("[ROI] 첫 프레임 읽기 실패")
        return None
    ...
    def mouse_callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            cv2.circle(init_frame, (x, y), 5, (0, 255, 0), -1)
            if len(selected_points) > 1:
                for i in range(1, len(selected_points)):
                    cv2.line(init_frame, selected_points[i-1], selected_points[i], (0, 255, 0), 2)
                if len(selected_points) > 2:
                    cv2.line(init_frame, selected_points[-1], selected_points[0], (0, 255, 0), 2)

    cv2.namedWindow("ROI Setup")
    cv2.setMouseCallback("ROI Setup", mouse_callback)
    print("[ROI] 폴리곤 점들을 클릭하고 'q'로 종료")
    while True:
        cv2.imshow("ROI Setup", init_frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    cv2.destroyWindow("ROI Setup")

    if len(selected_points) >= 3:
        print(f"[ROI] 설정 완료: {len(selected_points)}개 점")
        return np.array(selected_points, dtype=np.int32)
    else:
        print("[ROI] 점 부족 (3개 이상 필요). ROI 미적용.")
        return None
...
def main():
    ...
    with VideoReader(VIDEO_PATH) as video_reader:
        ...
        while True:
            ...
            # 첫 프레임 처리: YOLO로 탐지된 bbox를 SAM2에 등록 (ROI 안에 있는 것만)
            if frame_count == 1:
                sam2_predictor.load_first_frame(frame_rgb)
                results = model.predict(frame, verbose=False)
                for box in results[0].boxes:
                    x1, y1, x2, y2 = box.xyxy[0].cpu().numpy().astype(int)
                    if not is_bbox_in_roi([x1, y1, x2, y2], polygon): # ROI 밖이면 등록 안 함
                        continue
            ...
            # 이후 프레임 처리: SAM2로 객체 추적, YOLO로 신규 객체 탐지 및 후보 관리
                ...
                for box in results[0].boxes:
                    x1, y1, x2, y2 = box.xyxy[0].cpu().numpy().astype(int)
                    if not is_bbox_in_roi([x1, y1, x2, y2], polygon): # ROI 밖 검출은 무시
                        continue
                ...
                # ROI 밖으로 나간 추적 obj 자동 제거 (bbox 중심이 polygon 밖이면 즉시 to_remove)
                if polygon is not None:
                    for oid, mask in masks_by_id.items():
                        obj_bbox = get_bbox_from_mask(mask)
                        if not is_bbox_in_roi(obj_bbox, polygon):
                            to_remove.add(oid)
                ...
                for oid, mask in masks_by_id.items():
                    if oid in to_remove:                          # ROI 밖이거나 만료된 obj는 그리지 않음
                        continue
